...
resource "aws_instance" "db" {
  ...
  subnet_id                   = element(var.private_subnets,count.index) #module.webserver_vpc.private_subnets_id[count.index]
...
# Create a security group assigned to DB instances
...
# Create security group rules to allow ingress ssh traffic from VPC, and egress from all destinations 
resource "aws_security_group_rule" "db-ssh-ing" {
  ...
  to_port           = 22
  protocol          = "tcp"
  cidr_blocks       = ["${data.aws_vpc.whiskey-vpc.cidr_block}"]
